...
// Test this for linux with `LD_PRELOAD=$(pwd)/target/debug/libpreload_rs.so bash -c "echo the env var value is \'\$SOMEVAR\'"`
extern "C" fn ld_preload_initialise_fn() {
    // Did some test with a panicking function. See notes in `README.md`.
    // panic!("LET'S FAIL!");
